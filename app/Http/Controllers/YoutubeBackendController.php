 ...
 ///Backend Login---------------
   ...
   return view('backend.users_youtube.his_created',[
    ...
    public function users_store(Request $r){
        ...
        $acc=null;
        $item=new users();
        $ca=users::whereNotNull('type_youtube')->whereNotNull('email')->where('email',$r->email)->orderby('id','desc')->first();
    ...
    public function users_update(Request $r,$id){
        $item=users::where('id',$id)->first();
        $ca=users::whereNotNull('type_youtube')->whereNotNull('email')->where('id','!=',$id)->where('email',$r->email)->orderby('id','desc')->first();
        if($ca!=null){
                return redirect()->back()->with('message','Email Already Have in Data!');
            }
